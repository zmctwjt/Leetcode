...
# 
#  Related Topics 栈 设计 链表 字符串 双向链表 模拟 👍 75 👎 0
...
class TextEditor:
    ...
    def deleteText(self, k: int) -> int:
        l = min(len(self.left),k)
        del self.left[-l:]
        return l

    def cursorLeft(self, k: int) -> str:
        l = min(len(self.left),k)
        self.right += self.left[-1:-l-1:-1]
        del self.left[-1:-l-1:-1]
        return ''.join(self.left[-min(10,len(self.left)):])

    def cursorRight(self, k: int) -> str:
        r = min(len(self.right),k)
        self.left += self.right[-1:-r-1:-1]
        del self.right[-1:-r-1:-1]
        return ''.join(self.left[-min(10,len(self.left)):])
