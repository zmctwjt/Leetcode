...
#  
# 
#  Related Topics 贪心 数组 数学 二分查找 排序 堆（优先队列） 👍 101 👎 0
...
class Solution:
    ...
    def maxProfit(self, inventory: List[int], orders: int) -> int:
        ...
        ans = 0
        while orders:
            if inventory[0] - inventory[max_k]>=orders:
                ans += self.cumulative(inventory[0]) - self.cumulative(inventory[0] - orders)
                return ans
            else:
                orders -= (inventory[0] - inventory[max_k]) * max_k
                ans += (self.cumulative(inventory[0]) - self.cumulative(inventory[max_k])) * max_k
                max_k += 1
        return ans
