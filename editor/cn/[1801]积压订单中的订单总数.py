...
#  
# 
#  Related Topics 数组 模拟 堆（优先队列） 👍 116 👎 0
...
class Solution:
    def getNumberOfBacklogOrders(self, orders: List[List[int]]) -> int:
        ...
        for price,amount,ordertype in orders:
            if ordertype:
                while buy_q and -buy_q[0][0] >= price and amount:
                    if amount >= buy_q[0][1]:
                        amount -= buy_q[0][1]
                        heappop(buy_q)
                    else:
                    ...
                    if amount:
                        heappush(sell_q,[price,amount])
            else:
                while sell_q and sell_q[0][0] <= price and amount:
                    if amount >= sell_q[0][1]:
                        amount -= sell_q[0][1]
                        heappop(sell_q)
                    else:
                        sell_q[0][1] -= amount
                    ...
                    if amount:
                        heappush(buy_q,[-price,amount])
        return (sum(c for p,c in buy_q)+sum(c for p,c in sell_q))%MOD
