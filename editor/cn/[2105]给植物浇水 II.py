...
class Solution:
    def minimumRefill(self, plants: List[int], capacityA: int, capacityB: int) -> int:
        ...
        bob = len(plants)-1
        ans = 0
        remaining_waterA = capacityA
        remaining_waterB = capacityB
        while alice <bob:
            remaining_waterA -= plants[alice]
            remaining_waterB -= plants[bob]
            if remaining_waterA < 0:
                ans += 1
                remaining_waterA = capacityA-plants[alice]
            if remaining_waterB < 0:
                ans += 1
                remaining_waterB = capacityB-plants[bob]
            alice += 1
            bob -= 1
        if alice == bob:
            if remaining_waterA >= remaining_waterB:
                remaining_waterA -= plants[alice]
                if remaining_waterA < 0:
                    ans += 1
            else:
