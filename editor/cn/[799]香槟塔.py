...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        if query_glass == 0 and query_row == 0:
            return min(1,poured)
        glass = [[-1]*i for i in range(1,query_row+2)]
        glass[0][0] += poured
        for r,row in enumerate(glass):
            for c,col in enumerate(row):
                if r > 0:
                    right = left = 0
                    if c > 0 and glass[r-1][c-1] > 0:
                        left = glass[r-1][c-1] / 2
                    if c < len(row)-1 and glass[r-1][c]>0:
                        right = glass[r-1][c] / 2
